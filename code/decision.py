...
def decision_step(Rover):
    ...
    if Rover.mode == 'forward':
        # how far can we see
        if np.mean(Rover.nav_dists) > 300:
            Rover.throttle_set = 0.5
            Rover.max_vel = 4
        elif np.mean(Rover.nav_dists) > 100:
        ...
        if sum(angles_in_degree > 0)/float(angles_in_degree.size) > 0.9 and sum((angles_in_degree > 0) & (angles_in_degree < 5)) > 10:
            Rover.steer = 0
